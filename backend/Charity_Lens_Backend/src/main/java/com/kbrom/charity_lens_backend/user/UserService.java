...
import com.kbrom.charity_lens_backend.user.donorProfile.DonorProfileRepository;
...
public class UserService {
      ...
      public void changePassword(Long id, ChangePasswordDTO changePasswordDTO, Principal principal) {
          ...
          if(!isAdmin&&!currentUsername.equals(user.getUsername())) {
              throw new AccessDeniedException("You don't have permission to change another user's password");
          }
         if(!isAdmin&&!passwordConfirmed(changePasswordDTO,user)){
             throw new IllegalArgumentException("Incorrect password or new passwords do not match");
         }
